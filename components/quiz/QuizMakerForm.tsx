  ...
  return (
    <div className="p-8 max-w-3xl mx-auto">
      <div className="mb-8">
        ...
        <p className="text-gray-400 mt-1">Add official quizzes to the platform</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        <div className="bg-white/5 border border-white/10 rounded-2xl p-6 space-y-4">
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 resize-none"
            />
          </div>
          ...
          {questions.map((q, qIdx) => (
            <div key={qIdx} className="bg-white/5 border border-white/10 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                {questions.length > 1 && (
                  <button type="button" onClick={() => removeQuestion(qIdx)} className="text-red-400 hover:text-red-300 text-sm">
                    Remove
                  </button>
                )}
              </div>
              <input
                ...
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 mb-4"
              />
                  ...
                  <div key={oIdx} className="flex items-center gap-3">
                    <button
                      ...
                      className={`w-7 h-7 rounded-full border-2 flex-shrink-0 transition-colors ${
                        q.correctIndex === oIdx ? "bg-green-500 border-green-500" : "border-gray-600 hover:border-green-500"
                      }`}
                    />
                    <input
                      value={opt}
                      ...
                      className="flex-1 bg-white/5 border border-white/10 rounded-xl px-4 py-2.5 text-white placeholder-gray-600 focus:outline-none focus:border-purple-500 text-sm"
                    />
                  </div>
                ))}
              </div>
        ...
        <button
          type="button"
          onClick={addQuestion}
          className="w-full py-3 border border-dashed border-white/20 text-gray-400 hover:text-white hover:border-purple-500/50 rounded-xl transition-all text-sm"
        >
          + Add Question
        </button>
        ...
        <button
          type="submit"
          disabled={submitting}
          className="w-full py-4 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-500 hover:to-pink-500 disabled:opacity-50 text-white font-bold rounded-xl transition-all hover:scale-[1.01]"
        >
